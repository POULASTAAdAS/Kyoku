...
if !errorlevel! equ 0 (
    ...
    docker exec user-replica1 mysql -uroot -p%MYSQL_ROOT_USER_PASSWORD% -e "SHOW REPLICA STATUS\G" | findstr /C:"Replica_IO_Running" /C:"Replica_SQL_Running" /C:"Last_Error"
)
echo.

echo [INFO] Checking replica 2...
docker exec user-replica2 mysql -uroot -p%MYSQL_ROOT_USER_PASSWORD% USER -e "SELECT * FROM UserType WHERE type = '%test_value%';"
if !errorlevel! equ 0 (
...
docker exec user-replica3 mysql -uroot -p%MYSQL_ROOT_USER_PASSWORD% USER -e "SELECT * FROM UserType WHERE type = '%test_value%';"
if !errorlevel! equ 0 (
...
docker exec user-primary mysql -uroot -p%MYSQL_ROOT_USER_PASSWORD% USER -e "DELETE FROM UserType WHERE type = '%test_value%';"
