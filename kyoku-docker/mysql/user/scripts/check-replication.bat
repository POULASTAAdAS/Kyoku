...
echo           Master Status
echo ----------------------------------------
docker exec user-primary mysql -uroot -p%MYSQL_ROOT_USER_PASSWORD% -e "SHOW MASTER STATUS;"
...
echo ----------------------------------------
docker exec user-replica2 mysql -uroot -p%MYSQL_ROOT_USER_PASSWORD% -e "SHOW REPLICA STATUS\G" | findstr /C:"Replica_IO_Running" /C:"Replica_SQL_Running" /C:"Seconds_Behind_Source" /C:"Last_Error"

echo.
echo ----------------------------------------
echo         Replica 3 Status
echo ----------------------------------------
docker exec user-replica3 mysql -uroot -p%MYSQL_ROOT_USER_PASSWORD% -e "SHOW REPLICA STATUS\G" | findstr /C:"Replica_IO_Running" /C:"Replica_SQL_Running" /C:"Seconds_Behind_Source" /C:"Last_Error"
