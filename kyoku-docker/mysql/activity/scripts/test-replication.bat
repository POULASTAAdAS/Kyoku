...
docker exec activity-primary mysql -uroot -p%MYSQL_ROOT_ACTIVITY_PASSWORD% ACTIVITY -e "INSERT INTO UserPinnedType (id,type) VALUES (5,'%test_value%');"

if !errorlevel! neq 0 (
    echo [ERROR] Failed to insert test data on primary
    REM CHANGE: Show table structure to debug column size issues
    echo [DEBUG] Checking UserPinnedType table structure...
    docker exec activity-primary mysql -uroot -p%MYSQL_ROOT_ACTIVITY_PASSWORD% ACTIVITY -e "DESCRIBE UserPinnedType;"
    pause
    exit /b 1
)
...
docker exec activity-replica1 mysql -uroot -p%MYSQL_ROOT_ACTIVITY_PASSWORD% ACTIVITY -e "SELECT * FROM UserPinnedType WHERE id = 5;"
if !errorlevel! equ 0 (
    ...
    docker exec activity-replica1 mysql -uroot -p%MYSQL_ROOT_ACTIVITY_PASSWORD% -e "SHOW REPLICA STATUS\G" | findstr /C:"Replica_IO_Running" /C:"Replica_SQL_Running" /C:"Last_Error"
)
echo.

echo [INFO] Checking replica 2...
docker exec activity-replica2 mysql -uroot -p%MYSQL_ROOT_ACTIVITY_PASSWORD% ACTIVITY -e "SELECT * FROM UserPinnedType WHERE id = 5;"
if !errorlevel! equ 0 (
...
docker exec activity-replica3 mysql -uroot -p%MYSQL_ROOT_ACTIVITY_PASSWORD% ACTIVITY -e "SELECT * FROM UserPinnedType WHERE id = 5;"
if !errorlevel! equ 0 (
...
docker exec activity-primary mysql -uroot -p%MYSQL_ROOT_ACTIVITY_PASSWORD% ACTIVITY -e "DELETE FROM UserPinnedType WHERE id = 5;"
