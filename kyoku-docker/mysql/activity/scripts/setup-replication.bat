...
echo [INFO] Root password: %MYSQL_ROOT_ACTIVITY_PASSWORD%
echo [INFO] Replication user: %MYSQL_ACTIVITY_REPLICATION_USER%
...
:test_primary
set /a attempts+=1
docker exec activity-primary mysql -uroot -p%MYSQL_ROOT_ACTIVITY_PASSWORD% -e "SELECT 1;" >nul 2>&1
if !errorlevel! equ 0 (
...
docker exec activity-primary mysql -uroot -p%MYSQL_ROOT_ACTIVITY_PASSWORD% -e "SELECT User, Host FROM mysql.user WHERE User = '%MYSQL_ACTIVITY_REPLICATION_USER%';"

echo [INFO] Getting binary log status...
docker exec activity-primary mysql -uroot -p%MYSQL_ROOT_ACTIVITY_PASSWORD% -e "SHOW BINARY LOG STATUS;"

echo [INFO] Checking GTID mode on primary...
docker exec activity-primary mysql -uroot -p%MYSQL_ROOT_ACTIVITY_PASSWORD% -e "SHOW VARIABLES LIKE 'gtid_mode';"

REM Setup each replica
call :setup_replica activity-replica1
call :setup_replica activity-replica2  
call :setup_replica activity-replica3
...
echo ================================================
call :check_replica_status activity-replica1
call :check_replica_status activity-replica2
call :check_replica_status activity-replica3
...
docker exec %replica_name% mysql -uroot -p%MYSQL_ROOT_ACTIVITY_PASSWORD% -e "SELECT 1;" >nul 2>&1
if !errorlevel! equ 0 (
...
docker exec %replica_name% mysql -uroot -p%MYSQL_ROOT_ACTIVITY_PASSWORD% -e "SHOW VARIABLES LIKE 'gtid_mode';"
...
docker exec %replica_name% mysql -uroot -p%MYSQL_ROOT_ACTIVITY_PASSWORD% -e "STOP REPLICA; RESET REPLICA ALL; CHANGE REPLICATION SOURCE TO SOURCE_HOST='activity-primary', SOURCE_USER='%MYSQL_ACTIVITY_REPLICATION_USER%', SOURCE_PASSWORD='%MYSQL_ACTIVITY_REPLICATION_PASSWORD%', SOURCE_AUTO_POSITION=1, GET_SOURCE_PUBLIC_KEY=1; START REPLICA;"
...
if !errorlevel! equ 0 (
    ...
    docker exec %replica_name% mysql -uroot -p%MYSQL_ROOT_ACTIVITY_PASSWORD% -e "SHOW REPLICA STATUS\G" | findstr /C:"Last_Error"
)
goto :eof
...
docker exec %replica_name% mysql -uroot -p%MYSQL_ROOT_ACTIVITY_PASSWORD% -e "SHOW REPLICA STATUS\G" | findstr /C:"Replica_IO_Running" /C:"Replica_SQL_Running" /C:"Seconds_Behind_Source" /C:"Last_Error"
echo.
goto :eof