...
echo [INFO] Root password: %MYSQL_ROOT_CONTENT_PASSWORD%
echo [INFO] Replication user: %MYSQL_CONTENT_REPLICATION_USER%
...
:test_primary
set /a attempts+=1
docker exec content-primary mysql -uroot -p%MYSQL_ROOT_CONTENT_PASSWORD% -e "SELECT 1;" >nul 2>&1
if !errorlevel! equ 0 (
...
docker exec content-primary mysql -uroot -p%MYSQL_ROOT_CONTENT_PASSWORD% -e "SELECT User, Host FROM mysql.user WHERE User = '%MYSQL_CONTENT_REPLICATION_USER%';"
...
docker exec content-primary mysql -uroot -p%MYSQL_ROOT_CONTENT_PASSWORD% -e "SHOW BINARY LOG STATUS;"

REM CHANGE: Also show GTID status to verify GTID mode is working
echo [INFO] Checking GTID mode on primary...
docker exec content-primary mysql -uroot -p%MYSQL_ROOT_CONTENT_PASSWORD% -e "SHOW VARIABLES LIKE 'gtid_mode';"

REM Setup each replica
call :setup_replica content-replica1
call :setup_replica content-replica2  
call :setup_replica content-replica3
call :setup_replica content-replica4
call :setup_replica content-replica5
...
echo ================================================
call :check_replica_status content-replica1
call :check_replica_status content-replica2
call :check_replica_status content-replica3
call :check_replica_status content-replica4
call :check_replica_status content-replica5
...
docker exec %replica_name% mysql -uroot -p%MYSQL_ROOT_CONTENT_PASSWORD% -e "SELECT 1;" >nul 2>&1
if !errorlevel! equ 0 (
...
docker exec %replica_name% mysql -uroot -p%MYSQL_ROOT_CONTENT_PASSWORD% -e "SHOW VARIABLES LIKE 'gtid_mode';"
...
docker exec %replica_name% mysql -uroot -p%MYSQL_ROOT_CONTENT_PASSWORD% -e "STOP REPLICA; RESET REPLICA ALL; CHANGE REPLICATION SOURCE TO SOURCE_HOST='content-primary', SOURCE_USER='%MYSQL_CONTENT_REPLICATION_USER%', SOURCE_PASSWORD='%MYSQL_PLAYLIST_REPLICATION_PASSWORD%', SOURCE_AUTO_POSITION=1, GET_SOURCE_PUBLIC_KEY=1; START REPLICA;"
...
if !errorlevel! equ 0 (
    ...
    docker exec %replica_name% mysql -uroot -p%MYSQL_ROOT_CONTENT_PASSWORD% -e "SHOW REPLICA STATUS\G" | findstr /C:"Last_Error"
)
goto :eof
...
docker exec %replica_name% mysql -uroot -p%MYSQL_ROOT_CONTENT_PASSWORD% -e "SHOW REPLICA STATUS\G" | findstr /C:"Replica_IO_Running" /C:"Replica_SQL_Running" /C:"Seconds_Behind_Source" /C:"Last_Error"
echo.
goto :eof