...
echo [INFO] Root password: %MYSQL_ROOT_PLAYLIST_PASSWORD%
echo [INFO] Replication user: %MYSQL_PLAYLIST_REPLICATION_USER%
...
:test_primary
set /a attempts+=1
docker exec playlist-primary mysql -uroot -p%MYSQL_ROOT_PLAYLIST_PASSWORD% -e "SELECT 1;" >nul 2>&1
if !errorlevel! equ 0 (
...
docker exec playlist-primary mysql -uroot -p%MYSQL_ROOT_PLAYLIST_PASSWORD% -e "SHOW BINARY LOG STATUS;"

REM CHANGE: Also show GTID status to verify GTID mode is working
echo [INFO] Checking GTID mode on primary...
docker exec playlist-primary mysql -uroot -p%MYSQL_ROOT_PLAYLIST_PASSWORD% -e "SHOW VARIABLES LIKE 'gtid_mode';"

REM Setup each replica
call :setup_replica playlist-replica1
call :setup_replica playlist-replica2  
call :setup_replica playlist-replica3
call :setup_replica playlist-replica4
...
echo ================================================
call :check_replica_status playlist-replica1
call :check_replica_status playlist-replica2
call :check_replica_status playlist-replica3
call :check_replica_status playlist-replica4
...
docker exec %replica_name% mysql -uroot -p%MYSQL_ROOT_PLAYLIST_PASSWORD% -e "SELECT 1;" >nul 2>&1
if !errorlevel! equ 0 (
...
docker exec %replica_name% mysql -uroot -p%MYSQL_ROOT_PLAYLIST_PASSWORD% -e "SHOW VARIABLES LIKE 'gtid_mode';"
...
if !errorlevel! equ 0 (
    ...
    docker exec %replica_name% mysql -uroot -p%MYSQL_ROOT_PLAYLIST_PASSWORD% -e "SHOW REPLICA STATUS\G" | findstr /C:"Last_Error"
)
goto :eof
...
docker exec %replica_name% mysql -uroot -p%MYSQL_ROOT_PLAYLIST_PASSWORD% -e "SHOW REPLICA STATUS\G" | findstr /C:"Replica_IO_Running" /C:"Replica_SQL_Running" /C:"Seconds_Behind_Source" /C:"Last_Error"
echo.
goto :eof