...
class ServiceRepositoryImpl @Inject constructor(
    ...
    override suspend fun addSongToFavourite(songId: Long): ResponseSong {
        return try {
            api.addSongToFavourite(songId)
        } catch (e: Exception) {
            ResponseSong()
        }
    }

    override suspend fun removeFromFavourite(songId: Long): Boolean {
        return try {
            api.removeFromFavourite(songId)
        } catch (e: Exception) {
            false
        }
    }

    override suspend fun addSongToPlaylist(req: AddSongToPlaylistReq): ResponseSong {
        return try {
            api.addSongToPlaylist(req)
        } catch (e: Exception) {
            ResponseSong()
        }
    }
