...
class AppConfig {
    ...
    @Bean(destroyMethod = "") // disables Spring’s automatic close() inference. It is not an empty placeholder; the empty string specifically means “no destroy method.”
    fun provideRateLimitRedisConnection(
        factory: LettuceConnectionFactory,
    ): StatefulRedisConnection<String, ByteArray> {
        val client = factory.requiredNativeClient
        require(client is RedisClient) { "Auth rate limiting requires a standalone Redis connection" }

        return client.connect(RedisCodec.of(StringCodec.UTF8, ByteArrayCodec.INSTANCE))
    }

    @Bean
    fun provideRateLimitProxyManager(
        connection: StatefulRedisConnection<String, ByteArray>,
    ): ProxyManager<String> = Bucket4jLettuce.casBasedBuilder(connection)
